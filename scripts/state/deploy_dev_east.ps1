...
Write-Host "##vso[task.setvariable variable=CDP_DEFAULT_SCRIPTS]${CDP_SCRIPTS_PATH}/defaults"
Write-Host "##vso[task.setvariable variable=PRE_SCRIPT]${CDP_DEFAULT_SCRIPTS}/default.sh"
Write-Host "##vso[task.setvariable variable=POST_SCRIPT]${CDP_DEFAULT_SCRIPTS}/default.sh"
Write-Host "##vso[task.setvariable variable=OPENSHIFT_PREFIX]${APP_PLATFORM}-${APP_NAME}"
Write-Host "##vso[task.setvariable variable=OPENSHIFT_PROJECT]${APP_PLATFORM}-${APP_NAME}-${ENV}"
Write-Host "##vso[task.setvariable variable=APP_DEPLOY_ARGS]--cluster=${OPENSHIFT_CLUSTER}"
Write-Host "##vso[task.setvariable variable=APP_DEPLOY_COMMAND]${CDP_PROCESS_SCRIPTS}/deploy-openshift.sh"
Write-Host "##vso[task.setvariable variable=OPENSHIFT_CLUSTER]${ENV}-east.openshift-cluster.com"
